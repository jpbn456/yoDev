...
function authMax(env: Env): number {
...
function respond(response: Response, request: Request, env: Env): Response {
...
async function body(request: Request): Promise<JsonRecord | null> {
  try { return await request.json<JsonRecord>(); } catch { return null; }
}
...
async function uniqueSlug(firstName: string, lastName: string, env: Env): Promise<string> {
...
async function skillsFor(profileId: number, env: Env): Promise<{ name: string; slug: string }[]> {
...
async function resumeFor(profileId: number, env: Env): Promise<{ experiences: Experience[]; languages: Language[]; education: Education[] }> {
...
async function yearsForProfiles(profileIds: number[], env: Env): Promise<Map<number, number>> {
...
async function serialize(row: ProfileRow, env: Env, detail = false, privateFields = false, yearsExperience?: number): Promise<JsonRecord> {
...
async function profileForUser(userId: number, env: Env): Promise<ProfileRow | null> {
...
async function handle(request: Request, env: Env): Promise<Response> {
  ...
  if (path === "/api/profiles/" && method === "GET") {
    ...
    const clauses = ["p.is_published = 1"]; const values: unknown[] = [];
  ...
  async fetch(request, env): Promise<Response> {
    ...
    try {
      ...
      if (path.startsWith("/api/")) {
        ...
        // Inject per-profile meta tags for social crawlers that don't execute JS
        response = await injectProfileMeta(response, path, env);
      }
      ...
      return respond(response, request, env);
    } catch (cause) {
      console.error(JSON.stringify({ event: "request_error", requestId, method: request.method, path: new URL(request.url).pathname, cause: String(cause) }));
      return respond(error("Ocurrió un error inesperado.", 500), request, env);
    }
  },
} satisfies ExportedHandler<Env>;

async function injectProfileMeta(response: Response, path: string, env: Env): Promise<Response> {
  const match = path.match(/^\/developers\/([^/]+)\/?$/);
  if (!match) return response;
  const slug = match[1];
  const base = "https://yodev-api.primera-fila-s.workers.dev";
  ...
  const tags: [string, string][] = [
    ["<title>", `${title}</title>`],
    ["<meta name=\"description\"", `<meta name="description" content="${escapeAttribute(description)}"`],
    ["<meta property=\"og:title\"", `<meta property="og:title" content="${escapeAttribute(title)}"`],
    ["<meta property=\"og:description\"", `<meta property="og:description" content="${escapeAttribute(description)}"`],
    ["<meta property=\"og:url\"", `<meta property="og:url" content="${escapeAttribute(profileUrl)}"`],
    ["<meta property=\"og:type\"", `<meta property="og:type" content="profile"`],
    ["<meta property=\"profile:first_name\"", `<meta property="profile:first_name" content="${escapeAttribute(name.split(" ")[0] || "")}"`],
    ["<meta property=\"profile:last_name\"", `<meta property="profile:last_name" content="${escapeAttribute(name.split(" ").slice(1).join(" "))}"`],
    ["<meta name=\"twitter:title\"", `<meta name="twitter:title" content="${escapeAttribute(title)}"`],
    ["<meta name=\"twitter:description\"", `<meta name="twitter:description" content="${escapeAttribute(description)}"`],
    ["<link rel=\"canonical\"", `<link rel="canonical" href="${escapeAttribute(profileUrl)}"`],
  ];
  let result = html;
  // Replace <title> content first (it has no attributes, ends with </title>)
  const titleStart = result.indexOf("<title>");
  if (titleStart !== -1) {
    const titleEnd = result.indexOf("</title>", titleStart);
    if (titleEnd !== -1) {
      result = result.slice(0, titleStart + "<title>".length) + escapeContent(title) + result.slice(titleEnd);
    }
  }
  for (const [needle, replacement] of tags) {
    if (needle === "<title>") continue;
    const idx = result.indexOf(needle);
    if (idx === -1) continue;
    const endQuote = result.indexOf("\"", idx + needle.length);
    const endTag = result.indexOf(">", endQuote);
    if (endTag === -1) continue;
    result = result.slice(0, idx) + replacement + result.slice(endTag + 1);
  }
